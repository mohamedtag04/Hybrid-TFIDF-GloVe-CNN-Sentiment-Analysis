...
class HybridBertCNN(nn.Module):
    """Hybrid BERT-CNN model for sentiment analysis.

    Combines BERT embeddings with GloVe embeddings and applies CNN layers
    followed by multi-head attention and fully connected layers.

    Args:
        bert_model (BertModel): Pre-trained BERT model.
        glove_dim (int, optional): Dimension of GloVe embeddings. Defaults to 300.
    ...
    def forward(
        ...
        Args:
            ...
            glove_embeds (torch.Tensor): GloVe embeddings weighted by TF-IDF.

        Returns:
            torch.Tensor: Logits for sentiment classification (negative, positive).
        """
        with torch.no_grad():
